...
class PinActiveController extends Controller
{
    ...
    public function deactive($id, Request $request)
    {
        try {
            ...
            $updated = transection::where('id', $id)->first();
            $updated->tran_status = '2';
            $updated->update();

            $helpAmmount = $updated->get_ammount;
            if ($updated) {
                $totalProvideHelps = Provide_Help::where('status', '0')->where('transacted_status', '0')->with('getUsers')->where('users_id', '!=', $updated->user_id)->get();
                if (isset($totalProvideHelps) && count($totalProvideHelps) > 0) {
                        ...
                        if ($userlist->getUsers && !empty($userlist->getUsers)) {
                            if ($userlist->getUsers->user_type == "0") {
                                ...
                                if ($userlist->getUsers->getHelpAmmount > $userlist->getUsers->getAmmount && $helpAmmount > 0 && $userlist->getUsers->checkRemainingTransctions == 0) {
                                    ...
                                    $helpAmmount = $remainingHelpAmmount;
                                    $updateTransection = transection::where('id', $id)->first();
                                    $updateTransection->receiver_id = $userlist->getUsers->id;
                                    $updateTransection->tran_status = '0';
                                    $updateTransection->create_date = $currentDate;
                                    $updateTransection->end_date = $tomorrow;
                                    $updateTransection->update();
                ...
                if ($helpAmmount > 0) {
                    ...
                    if ($defaultUser) {
                        $defaultpayment = new transection();
                        $defaultpayment->user_id = $updated->sender_id;
                        $defaultpayment->sender_id = $updated->sender_id;
                        $defaultpayment->receiver_id = $defaultUser->id;
                        $defaultpayment->past_receiver_id = $defaultUser->unique_pin;
                        $defaultpayment->provide_help_id = $defaultUser->provideHelpUser->id;
                        $defaultpayment->tran_status = 0;
                        ...
                        $defaultpayment->get_ammount = $helpAmmount;
                        $defaultpayment->split_status = '1';
                        $defaultpayment->pin_number = $updated->pin_number;
                        $defaultpayment->save();
                    }
                }

                $updateProvideHelp = Provide_Help::where('id', $updated->provide_help_id)->first();
                $updateProvideHelp->transacted_status = '0';
                $updateProvideHelp->update();
            }
